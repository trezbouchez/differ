/*
    MovingAverageRollingHasher

    Implements very simple moving average (sliding window) rolling hash
...
pub(crate) struct MovingAverageRollingHasher {
    rolling_hash: u32,
...
impl RollingHasher for MovingAverageRollingHasher {
    ...
    fn push(&mut self, byte: u8) -> u32 {
        ...
        self.rolling_hash = self.rolling_hash.overflowing_add(byte_entering_window).0.overflowing_sub(byte_exiting_window).0;
...
impl MovingAverageRollingHasher {
...
fn test_moving_average_rolling_hash_wrong_window_size() {
    let _ = MovingAverageRollingHasher::new(31);
}

#[test]
fn test_moving_average_rolling_hash() {    
    // trying some basic sequence first
    let mut hasher = MovingAverageRollingHasher::new(4);
    ...
    let mut hasher = MovingAverageRollingHasher::new(16);
    ...
    let mut hasher = MovingAverageRollingHasher::new(2);
